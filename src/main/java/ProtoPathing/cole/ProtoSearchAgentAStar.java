...
public class ProtoSearchAgentAStar {
	...
	public ProtoSearchAgentAStar() {
		...
		while (!newSearchList.isEmpty()) {
			...
			if (gameState.state.goalFound) {
				...
				for (RankedState ls : getPathToNode(gameState))
					System.out.println(ls.state);
				return;
			}
	...
	private RankedState gen(Directions direction) {
		try {
			...
			switch (direction) {
			case UP:
				temp = new RankedState(this.gameState.state.moveUp());
				break;
			case DOWN:
				temp = new RankedState(this.gameState.state.moveDown());
				break;
			case LEFT:
				temp = new RankedState(this.gameState.state.moveLeft());
				break;
			case RIGHT:
				temp = new RankedState(this.gameState.state.moveRight());
				break;
			default:
				temp = null;
			}
